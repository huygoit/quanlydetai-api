...
export default class PublicationsController {
  ...
  async index({ auth, request, response }: HttpContext) {
    ...
    const q = Publication.query().where('profile_id', profile.id).orderBy('year', 'desc').orderBy('id', 'desc')
  ...
  async store({ auth, request, response }: HttpContext) {
    ...
    return response.created({
      success: true,
      data: { id: pub.id, title: pub.title, publicationType: pub.publicationType, year: pub.year, createdAt: pub.createdAt.toISO() },
    })
  }
  ...
  async update({ auth, params, request, response }: HttpContext) {
    ...
    await pub.save()
    await this.updateProfileCompleteness(profile.id)
    return response.ok({ success: true, data: { id: pub.id, title: pub.title, publicationType: pub.publicationType, year: pub.year } })
  }
